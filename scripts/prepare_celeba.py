...
def get_condition_config(dataset_name: str, dataset_attr_names=None):
    if dataset_name == "two_class_color_mnist":
        ...
        _temp = {
            ...
            # dataset_attr_names[9]: True,  # Blond_Hair
...
def generate_imgs_classes_contents(train: bool = True):
    ...
    for i in range(len(dataset)):
        ...
        classes.append(np.array(data[1][15]))
        contents.append(np.array(data[1][9]))
...
if __name__ == '__main__':
    ...
    np.savez(join(save_folder, f'celeba_15cls_9ctt_train.npz'),
             imgs=train_imgs,
    ...
    np.savez(join(save_folder, f'celeba_15cls_9ctt_test.npz'),
             imgs=test_imgs,
