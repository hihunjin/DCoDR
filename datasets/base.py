...
def sample_index(
    ...
    num_samples: int,
    per_attr: bool = True,  # if False, sample from all attributes (if num_samples is int)
    seed: int = 0,
    sort: bool = True,
) -> Tensor:
    ...
    if per_attr:
        ...
        for values in product(*values_per_attr):
            ...
            _indices = torch.randperm(candidates.size(0), generator=rng)[:num_samples]
            indicies_list.append(candidates[_indices])

        indices = torch.cat(indicies_list)

    else:
        indices = torch.randperm(attrs.size(0), generator=rng)[:num_samples]
